...
# --- Core pipeline --- AAPL, TSLA, INTC, PLTR, GLD, AMZN, KO, NKE
def compute_smile(ticker="INTC", expiry="2025-12-19", r=0.05, q=0.0):
    try:
        ...
        if len(df) < 6:   # need enough points for a curve fit
            raise RuntimeError(f"Not enough valid IV points for {ticker} {expiry} (got {len(df)})")
        ...
        return df[["logk", "iv"]], pd.DataFrame({"logk":k_grid, "iv": iv_fit})
    except Exception:
        return None

def usable_expiries(ticker, n = 8):
    tkr = yf.Ticker(ticker)
    good = []
    for exp in tkr.options[:n]:
        res = compute_smile(ticker, exp)
        if res is not None:
            good.append(exp)
    return good